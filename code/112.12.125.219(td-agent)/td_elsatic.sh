...
kill `ps -ef |grep td-agent-pwaf | grep -v grep |awk '{print $2}'`
kill `ps -ef |grep td-agent-pwaf | grep -v grep |awk '{print $2}'`
kill `ps -ef |grep td-agent-pwaf | grep -v grep |awk '{print $2}'`
sleep 3
#sleep 30
/opt/td-agent/embedded/bin/ruby /usr/sbin/td-agent-pwaf --group td-agent --log /var/log/td-agent/td-agent-pwaf.log --use-v1-config --daemon /var/run/td-agent/td-agent-pwaf.pid
