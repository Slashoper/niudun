...
# at  : 2016-08-22
# change: 20160830 add total percentage of 404 
...
query_domain = ["*81.cn", "*mod.gov.cn", "*chinamil.com.cn"]
query_template = '''
...
def Query_Results(write_result) :
    ...
    result = open('/tmp/404_resut_send_qq.log', 'w')
    date = time.strftime("%Y-%m-%d %H:%M:%S")
    result.write("%s\n"%date)
    for domain, http_404_count in write_result.iteritems():
        #print domain, http_5xx_count
        if len(http_404_count) > 1 :
            domain_write_flag = True
            for i in http_404_count[0:-1] :
                doc_count = i.get('doc_count')
                num_count = float(http_404_count[-1])
                total_percentage_404 = doc_count/num_count*100 
                if (doc_count >= 300) or (total_percentage_404 >= 3):
                     if domain_write_flag :
                         result.write("==== %s\n"%domain)
                         domain_write_flag = False
                     result.write("%s    %s    %.2f%%\n"%(i.get('key'), doc_count, total_percentage_404))
            #result.write("\n")
    result.flush()
    result.close()
...
def Query_Results_Send_QQ() :
    '''发送QQ消息'''
    #return
    fp_result = open('/tmp/404_resut_send_qq.log', 'r')
    msg_backup = open('/var/log/elastich_404.log', 'a+')
