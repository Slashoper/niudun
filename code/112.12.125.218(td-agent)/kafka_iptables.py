...
def get_cmdb_ip():
    try :
        cmd = '/usr/bin/curl -d "grant_type=client_credentials&client_id=testclient&client_secret=testsecret" http://61.174.9.72/oauth/token'
        res = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        token  = json.loads(res.stdout.read())
        token = token.get('access_token')
        headers = {'NDAUTHTOKEN':token}
        url_assets = 'http://61.174.9.72/api/assets/view'
